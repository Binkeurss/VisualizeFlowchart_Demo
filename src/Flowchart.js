class Flowchart
{
	...
	constructor(positions, config, canvasID)
	{
		...
		let canvasWidth = this.#initialX*2+(positions[0].length*(config.rectangleWidth+config.lineWidth + 20))-config.lineWidth;
	...
	//Custom
	parallelogram(title, color) {
		const x = this.#elements[title].x;
		const y = this.#elements[title].y;
		this.#context.drawParallelogram(x, y, color);		
		this.#context.drawText(title, x + (this.#config.rectangleWidth / 2), y + (this.#config.rectangleHeight / 2), this.#config.rectangleWidth);
	}

	oval(title, color = "red") {
		const x = this.#elements[title].x;
		const y = this.#elements[title].y;
